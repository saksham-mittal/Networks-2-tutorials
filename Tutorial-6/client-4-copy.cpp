...
void receiveObject(int size, int sockid, string hostName, string filePath) {
...
void downloadWesite(int thID, string hostName, vector<string> objPaths) {
    ...
    while(j < objPaths.size()) {
        ...
        printf("Thread %d downloading the object %s\n", thID, (char*)objPaths[j].c_str());
        ...
        try
        {
            if(send(sockid, headRequest, strlen(headRequest), 0) < 0)
                ...
                receiveObject(size, sockid, hostName, filePath);

                printf("Writing to file %s completed by thread %d\n", (char*)filePath.c_str(), thID);
            }
        }
        catch(...) {
            continue;
